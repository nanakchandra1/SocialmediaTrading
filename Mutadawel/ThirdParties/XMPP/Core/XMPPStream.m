...
- (BOOL)registerWithPassword:(NSString *)password Email:(NSString*)email Name:(NSString*)name error:(NSError **)errPtr{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        [queryElement addChild:[NSXMLElement elementWithName:@"name" stringValue:name]];
        [queryElement addChild:[NSXMLElement elementWithName:@"id" stringValue:@"sdsdsds"]];
...
- (BOOL)registerWithElements:(NSArray *)elements error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		XMPPIQ *iq = [XMPPIQ iqWithType:@"set" elementID:@"12"];
