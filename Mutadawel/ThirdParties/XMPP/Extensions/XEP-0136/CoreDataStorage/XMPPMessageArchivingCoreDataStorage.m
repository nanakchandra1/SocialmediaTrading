...
- (void)didUpdateContact:(XMPPMessageArchiving_Contact_CoreDataObject *)contact
{
	// Override hook
    NSError *err ;
    if ([contact.managedObjectContext hasChanges]) {
        if ([contact.managedObjectContext save:&err]) {
            NSLog(@"%@",err);
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ([self.mainThreadManagedObjectContext save:&err1]) {
                    NSLog(@"%@",err1);
...
- (XMPPMessageArchiving_Contact_CoreDataObject *)contactForMessage:(XMPPMessageArchiving_Message_CoreDataObject *)msg
{
    ...
    if (msg.message.outOfBandBusinessId.length != 0) {
        return [self contactWithBareJidStr:msg.bareJidStr
                          streamBareJidStr:msg.streamBareJidStr
                      managedObjectContext:msg.managedObjectContext ids_string:msg.message.outOfBandBusinessId];
...
- (XMPPMessageArchiving_Contact_CoreDataObject *)contactWithBareJidStr:(NSString *)contactBareJidStr
                                                      streamBareJidStr:(NSString *)streamBareJidStr
                                                  managedObjectContext:(NSManagedObjectContext *)moc ids_string:(NSString*)ids_string
{
    NSEntityDescription *entity = [self contactEntity:moc];
    ...
    if (streamBareJidStr)
    {
        //predicate = [NSPredicate predicateWithFormat:@"bareJidStr == %@ AND streamBareJidStr == %@",
        //           contactBareJidStr, streamBareJidStr];
        NSPredicate *predicate1 = [NSPredicate predicateWithFormat:@"ids == %@ AND streamBareJidStr == %@", ids_string,
                                   streamBareJidStr];
        NSPredicate *predicate2 = [NSPredicate predicateWithFormat:@"bareJidStr == %@", contactBareJidStr];
        predicate = [NSCompoundPredicate andPredicateWithSubpredicates:@[predicate1,predicate2 ]];
    }
    else
    {
        predicate = [NSPredicate predicateWithFormat:@"ids == %@", ids_string];
    }
	...
	[self scheduleBlock:^{
		...
		else
		{
            ...
            if (timestamp){
                ...
                archivedMessage.sortedTimeStamp=[[NSDate alloc] init];
            }
            else{
            ...
            if (isOutgoing) {
                if (message.elementID != nil) {
                    archivedMessage.element_id = message.elementID ;
                }else{
                    archivedMessage.element_id = @"xyz" ;

                }
                archivedMessage.markStatus = [NSNumber numberWithInt: 0];
            }else{
                if (message.elementID != nil) {
                    archivedMessage.element_id = message.elementID ;
                }else{
			...
			if ([messageBody length] > 0)
			{
                ...
                contact.message = archivedMessage.message;
