...
- (void)addOutOfBandURI:(NSString *)URI desc:(NSString *)desc businessId:(NSString*)businessId
{
    ...
    if(businessId)
    {
        NSXMLElement *descElement = [NSXMLElement elementWithName:@"lbid" stringValue:businessId];
        [outOfBand addChild:descElement];
    }
    [self addChild:outOfBand];
}
...
{
    return ([self elementForName:NAME_OUT_OF_BAND xmlns:XMLNS_OUT_OF_BAND]);
}
- (NSURL *)outOfBandURL
{
	NSURL *URL = nil;
...
- (NSString *)outOfBandBusinessId{
    ...
    NSXMLElement *descElement = [outOfBand elementForName:@"lbid"];
