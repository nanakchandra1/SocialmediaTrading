...
public class SkyFloatingLabelTextFieldWithIcon: SkyFloatingLabelTextField {
    ...
    public var iconLabel:UIImageView!
    
    /// A UIFont value that determines the font that the icon is using
    @IBInspectable
    public var iconFont:UIImage? {
        didSet {
            self.iconLabel?.image = iconFont
        }
    }
    
    /// A String value that determines the text used when displaying the icon
    @IBInspectable
    public var iconText:UIImage? {
        didSet {
            self.iconLabel?.image = iconText
        }
    }
    ...
    */
    ...
    private func createIconLabel() {
        ...
        iconLabel.frame = CGRect(x: 0, y: 0, width: 25, height: 25)//(x: 0, y: 0, w: 25, h: 25)
        self.iconLabel = iconLabel
        self.iconLabel.contentMode = .center
    ...
    private func updateIconLabelColor() {
        if self.hasErrorMessage {
            self.iconLabel?.image = #imageLiteral(resourceName: "ic_create_profile_unfilled_lock")
        } else {
            self.iconLabel?.image = #imageLiteral(resourceName: "ic_create_profile_unfilled_lock")
        }
    }
