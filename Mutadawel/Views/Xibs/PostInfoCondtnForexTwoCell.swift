//
//  PostInfoCondtnForexTwoCell.swift
//  Mutadawel
//
//  Created by Appinventiv on 26/04/17.
...
class PostInfoCondtnForexTwoCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        self.priceLbl.text = PRICE.localized
    ...
    func populateView(index: IndexPath, info: ForecastPostDetailModel){
        ...
        if let from_time = info.from_time {
            self.fromTimeValue.text = "\(from_time)"
            
        }
        if let to_time = info.to_time {
            
            self.toTimeValue.text = "\(to_time)"
